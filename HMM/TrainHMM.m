function HMMModel = TrainHMM(TrainData, KMeansC, NIter, M, N, NumClasses, NumSamplesPerClassTrain, FilterName)
...
for  i = 1:NumClasses
    AAvg{i} = zeros(size(A));
    BAvg{i} = zeros(size(B));
    PiAvg{i} = zeros(size(Pi));
    for j = 1:NumSamplesPerClassTrain
        AAvg{i} = AAvg{i} + AAll{i,j};
        BAvg{i} = BAvg{i} + BAll{i,j};
        PiAvg{i} = PiAvg{i} + PiAll{i,j};
    end
    AAvg{i} = AAvg{i}./NumSamplesPerClassTrain;
    BAvg{i} = BAvg{i}./NumSamplesPerClassTrain;
    PiAvg{i} = PiAvg{i}./NumSamplesPerClassTrain;
end

HMMModel.A = AAvg;
HMMModel.B = BAvg;
HMMModel.Pi = PiAvg;
HMMModel.KMeansC = KMeansC;
HMMModel.FilterName = FilterName;

save(['HMMModelM',num2str(M),'N',num2str(N),FilterName,'.mat'],'HMMModel');
